...
<xsl:stylesheet xmlns="http://exist-db.org/collection-config/1.0"
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:index="http://aac.ac.at/content_repository/index"
    ...
    <xsl:param name="default-analyzer-class"
        >org.apache.lucene.analysis.standard.StandardAnalyzer</xsl:param>
    <xsl:param name="default-data-type">xs:string</xsl:param>
    <xsl:variable name="all-range-indexes"
        select="//index[@type = ('default', 'range') or not(@type)]"/>
    <xsl:variable name="range-indexes" as="item()*">
        ...
        <xsl:for-each select="
                distinct-values($all-range-indexes/path/(if (@match) then
                    @match
                else
                    text())/index:qnamesFromPath(.))">
            <xsl:variable name="currentIndex" select="."/>
            <xsl:apply-templates select="
                    $all-range-indexes[some $x in (if (path/@match) then
                        path/@match
                    else
                        path/text())
                        satisfies index:qnamesFromPath($x) = $currentIndex][1]"/>
        </xsl:for-each>
    </xsl:variable>
    ...
    <xsl:template match="
            *[some $x in $all-range-indexes
                satisfies $x is .]">
        <xsl:variable name="current" select="."/>
        <xsl:variable name="indexes-with-same-qname" select="
                $all-range-indexes[some $x in (if (path/@match) then
                    path/@match
                else
                    path/text())
                    satisfies index:qnamesFromPath($x) = $current/(if (path/@match) then
                        path/@match
                    else
        ...
        <xsl:variable name="all-paths" as="item()*" select="
                distinct-values(path/index:qnamesFromPath((if (@match) then
                    @match
                else
                    text())))"/>
    ...
    <xsl:template match="
            *[some $x in $all-fulltext-indexes
                satisfies $x is .]">
