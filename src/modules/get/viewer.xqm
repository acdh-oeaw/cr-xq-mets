                ...
                switch ($type)
                    case 'data' return 
                        ...
                        then () (: we don't want to return the whole data of a proejct ... :)
                            ...
                            return 
                                if ($resource-pid='') then diag:diagnostics('record-does-not-exist','Could determine project-id from supplied id '||$id) else  
                                if ($is-restricted)   then diag:diagnostics('record-not-authorised-to-send',('access to the data of this resource is restriced. Please refer to its metadata for further information.')) else 
                                if (not($is-restricted instance of xs:boolean)) then diag:diagnostics('general-error','value "'||$is-restricted||'" is unsuitable to determine whether data access is restriced or not')  
