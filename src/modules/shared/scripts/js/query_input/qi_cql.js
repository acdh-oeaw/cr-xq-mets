...
!function ($, console) {
    ...
    module.genCQLInput = function (key, param_settings) {
        ...
        if (param_settings.size)
            $(new_input).attr("size", param_settings.size);
    ...
    function Query(key, param_settings) {
        ...
        this.removeSearchClause = function (source_clause) {
            ...
            if (!(and_pos == 0 && or_pos == 0)) {
                ...
                if (this.searchclauses[and_pos].length == 0)
                    this.searchclauses.splice(and_pos, 1);
        ...
        this.updateCQL = function () {
            ...
            for (var i = 0; i < this.searchclauses.length; i++) {
                if (i > 0)
                    cqltext = cqltext + " and ";
                if (this.searchclauses.length > 1)
                    cqltext = cqltext + " ( ";
                for (var j = 0; j < this.searchclauses[i].length; j++) {
                    if (j > 0)
                        cqltext = cqltext + " or ";
                    if (this.searchclauses[i].length > 1)
                        cqltext = cqltext + " ( ";
    ...
    function SearchClause(query_object, and_pos, or_pos) {
        ...
        this.genCQLSearchClauseWidget = function () {
            ...
            if (this.query_object.cql_config) {
                ...
                this.query_object.cql_config.onLoaded = function (index) {
                    input_value.autocomplete("search");
        ...
        this.genControls = function () {
            ...
            var me = this;
            cmd_del.bind("click", function (event) {
                me.query_object.removeSearchClause(me);
            });
            cmd_and.bind("click", function (event) {
                me.query_object.addSearchClause(me, "and");
            });
            cmd_or.bind("click", function (event) {
                me.query_object.addSearchClause(me, "or");
            });
