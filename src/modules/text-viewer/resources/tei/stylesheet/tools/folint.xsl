...
<xsl:stylesheet xmlns:fo="http://www.w3.org/1999/XSL/Format"
    ...
    xmlns:rx="http://www.renderx.com/XSL/Extensions" version="1.0" exclude-result-prefixes="fo rx">
    ...
    <xsl:template match="fo:root">
        <xsl:if test="@*[. = 'inherit']">
            <xsl:call-template name="print-warning">
                <xsl:with-param name="msg">Attributes with value of 'inherit' are meaningless on
        ...
        <xsl:apply-templates
            select="*[not(self::rx:meta-info or self::fo:layout-master-set or self::fo:declarations or self::rx:outline or self::fo:page-sequence)]"
            mode="report-intrusive-elements"/>
        <xsl:if test="not(fo:layout-master-set)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element 'layout-master-set' is required as a child of
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:layout-master-set) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:layout-master-set[1])"/>' element as a child of
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:declarations) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:declarations[1])"/>' element as a child of '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="not(fo:page-sequence)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There should be at least one 'page-sequence' element as a
                    child of '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:layout-master-set[preceding-sibling::fo:declarations]">
            <xsl:call-template name="print-warning">
                <xsl:with-param name="msg">Element '<xsl:value-of
                        select="name(fo:layout-master-set[1])"/>' shall precede '<xsl:value-of
                        select="name(fo:declarations[1])"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:declarations[preceding-sibling::fo:page-sequence]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name(fo:declarations[1])"
                    />' shall be placed before the first '<xsl:value-of
                        select="name(fo:page-sequence[1])"/>' element. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:layout-master-set[preceding-sibling::fo:page-sequence]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of
                        select="name(fo:layout-master-set[1])"/>' shall be placed before the first
                        '<xsl:value-of select="name(fo:page-sequence[1])"/>' element.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        ...
        <xsl:if test="count(rx:meta-info) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(rx:meta-info[1])"/>' element as a child of '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(rx:outline) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(rx:outline[1])"/>' element as a child of '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="rx:meta-info[preceding-sibling::*]">
            <xsl:call-template name="print-warning">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name(rx:meta-info[1])"/>'
                    should be the first child of '<xsl:value-of select="name()"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="rx:outline[preceding-sibling::fo:page-sequence]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name(rx:outline[1])"/>'
                    shall be placed before the first '<xsl:value-of
                        select="name(fo:page-sequence[1])"/>' element. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template match="fo:layout-master-set">
        ...
        <xsl:call-template name="no-text"/>
        <xsl:apply-templates
            select="*[not(self::fo:simple-page-master or self::fo:page-sequence-master)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'simple-page-master' and 'page-sequence-master'
                elements are permitted in this context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(fo:simple-page-master)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There should be at least one 'simple-page-master' element
                    inside '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="fo:simple-page-master">
        ...
        <xsl:call-template name="no-text"/>
        <xsl:apply-templates
            select="*[not(self::fo:region-body or self::fo:region-before or self::fo:region-after or self::fo:region-start or self::fo:region-end)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only region descriptors are permitted in this
                context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(@master-name)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'master-name' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:variable name="master-name" select="@master-name"/>
        <xsl:if
            test="ancestor::fo:layout-master-set and (following-sibling::*[@master-name = $master-name] or preceding-sibling::*[@master-name = $master-name])">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg"> Duplicate identifier: master-name="<xsl:value-of
                        select="$master-name"/>". Property 'master-name' should be unique within
                        '<xsl:value-of select="name(ancestor::fo:layout-master-set[1])"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="not(fo:region-body)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element 'region-body' is required inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:region-body) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:region-body[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:region-before) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:region-before[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:region-after) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:region-after[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:region-start) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:region-start[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:region-end) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:region-end[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        ...
        <xsl:if test="$strictness &gt; 0">
            <xsl:if
                test="fo:region-body/preceding-sibling::*[self::fo:region-before or self::fo:region-after or self::fo:region-start or self::fo:region-end] or fo:region-before/preceding-sibling::*[self::fo:region-after or self::fo:region-start or self::fo:region-end] or fo:region-after/preceding-sibling::*[self::fo:region-start or self::fo:region-end] or fo:region-start/preceding-sibling::fo:region-end">
                <xsl:call-template name="print-warning">
                    <xsl:with-param name="msg">Incorrect order of region descriptors inside
                            '<xsl:value-of select="name()"/>'. Regions shall be ordered according to
                        the following content model: fo:region-body, fo:region-before?,
                        fo:region-after?, fo:region-start?, fo:region-end?</xsl:with-param>
                </xsl:call-template>
            </xsl:if>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    ...
    <!-- Regions                             -->
    <xsl:template
        match="fo:region-body | fo:region-before | fo:region-after | fo:region-start | fo:region-end">
        <xsl:apply-templates select="@*"/>
        <xsl:call-template name="empty-element"/>
        <xsl:if test="not(@extent) and not(self::fo:region-body)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'extent' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        ...
        <xsl:if
            test="ancestor::fo:simple-page-master and (preceding-sibling::*[@region-name = $region-name] or following-sibling::*[@region-name = $region-name])">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg"> Duplicate identifier: region-name="<xsl:value-of
                        select="$region-name"/>". Property 'region-name' shall be unique within
                        '<xsl:value-of select="name(ancestor::fo:simple-page-master[1])"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="$strictness &gt; 1">
            <xsl:if
                test="@*[starts-with(local-name(), 'padding')][normalize-space(.) != '0' and normalize-space(.) != '0pt' and normalize-space(.) != '0px' and normalize-space(.) != '0mm' and normalize-space(.) != '0em' and normalize-space(.) != '0cm' and normalize-space(.) != '0pc' and normalize-space(.) != '0in']">
                <xsl:call-template name="print-warning">
                    <xsl:with-param name="msg"> Padding on '<xsl:value-of select="name()"/>' is a
                        RenderX extension. </xsl:with-param>
                </xsl:call-template>
            </xsl:if>
            <xsl:if test="@*[starts-with(local-name(), 'border')]">
                <xsl:call-template name="print-warning">
                    <xsl:with-param name="msg"> Border on '<xsl:value-of select="name()"/>' is a
                        RenderX extension. </xsl:with-param>
                </xsl:call-template>
            </xsl:if>
        </xsl:if>
    ...
    <xsl:template match="fo:page-sequence-master">
        ...
        <xsl:apply-templates
            select="*[not(self::fo:single-page-master-reference or self::fo:repeatable-page-master-reference or self::fo:repeatable-page-master-alternatives)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only page sequence specifiers are permitted in this
                context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if
            test="not(fo:single-page-master-reference or fo:repeatable-page-master-reference or fo:repeatable-page-master-alternatives)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">At least one page sequence specifier is required inside
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    ...
    <xsl:template
        match="fo:single-page-master-reference | fo:repeatable-page-master-reference | fo:conditional-page-master-reference">
        <xsl:apply-templates select="@*"/>
        <xsl:call-template name="empty-element"/>
        <xsl:if test="not(@master-reference)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'master-reference' is required for
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="fo:repeatable-page-master-alternatives">
        ...
        <xsl:apply-templates select="*[not(self::fo:conditional-page-master-reference)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'conditional-page-master-reference' elements are
                permitted in this context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(fo:conditional-page-master-reference)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">At least one 'conditional-page-master-reference' element
                    should be present inside '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="rx:meta-info">
        ...
        <xsl:apply-templates select="*[not(self::rx:meta-field)]" mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'meta-field' elements are permitted in this
                context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(rx:meta-field)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">At least one 'meta-field' element should be present
                    inside '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    ...
    <xsl:template match="rx:meta-field">
        ...
        <xsl:if test="not(@name)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'name' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="not(@value)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'value' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="rx:page-device" mode="report-intrusive-elements">
        <xsl:call-template name="print-error">
            <xsl:with-param name="msg">Element 'page-device' is obsolete; use &lt;?xep-postscript-*
                ?&gt; processing instructions instead.</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="fo:declarations">
        ...
        <xsl:apply-templates select="*[not(self::fo:color-profile)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'color-profile' elements are permitted in this
                context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="fo:color-profile">
        ...
        <xsl:if test="not(@color-profile-name)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'color-profile-name' is required for
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="not(@src)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'src' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="rx:outline">
        ...
        <xsl:apply-templates select="*[not(self::rx:bookmark)]" mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'bookmark' elements are permitted in this
                context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(rx:bookmark)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">At least one 'bookmark' element should be present inside
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    ...
    <xsl:template match="rx:bookmark">
        ...
        <xsl:if test="not(@internal-destination or @external-destination)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Either 'internal-destination' or 'external-destination'
                    attribute must be present on '<xsl:value-of select="name()"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="@internal-destination and @external-destination">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Only one of 'internal-destination' or
                    'external-destination' attributes can be present on '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*[not(self::rx:bookmark or self::rx:bookmark-label)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'bookmark-label' and/or nested 'boormark' elements
                are permitted in this context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(rx:bookmark-label)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element 'bookmark-label' is required inside
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(rx:bookmark-label) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There should be exactly one '<xsl:value-of
                        select="name(rx:bookmark-label[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="rx:bookmark-label[preceding-sibling::*]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of
                        select="name(rx:bookmark-label[1])"/>' should be the first child of
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    ...
    <xsl:template match="rx:bookmark-label">
        ...
        <xsl:apply-templates select="*" mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only plain text is permitted in this
                context.</xsl:with-param>
        </xsl:apply-templates>
    </xsl:template>
    ...
    <xsl:template match="fo:page-sequence">
        ...
        <xsl:apply-templates
            select="*[not(self::fo:title or self::fo:static-content or self::fo:flow)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'flow', 'static-content', and 'title' elements are
                permitted in this context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(fo:flow)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element 'flow' is required inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:flow) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:flow[1])"/>' element inside '<xsl:value-of select="name()"
                    />'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:title) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be at most one '<xsl:value-of
                        select="name(fo:title[1])"/>' element inside '<xsl:value-of select="name()"
                    />'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:title[preceding-sibling::*]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name(fo:title[1])"/>'
                    should be the first child of its parent '<xsl:value-of select="name()"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:flow[following-sibling::*]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name(fo:flow[1])"/>'
                    should be the last child of its parent '<xsl:value-of select="name()"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    ...
    <xsl:template match="fo:flow | fo:static-content">
        ...
        <xsl:if test="not(@flow-name)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'flow-name' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:variable name="flow-name" select="@flow-name"/>
        <xsl:if
            test="ancestor::fo:page-sequence and (following-sibling::*[self::fo:flow or self::fo:static-content][@flow-name = $flow-name] or preceding-sibling::*[self::fo:flow or self::fo:static-content][@flow-name = $flow-name])">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg"> Duplicate identifier: flow-name="<xsl:value-of
                        select="$flow-name"/>". Property 'flow-name' should be unique within
                        '<xsl:value-of select="name(ancestor::fo:page-sequence[1])"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    ...
    <xsl:template match="rx:flow-section">
        ...
        <xsl:if test="not(parent::fo:flow)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' must be a
                    direct descendant of 'flow'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="fo:bidi-override">
        ...
        <xsl:if test="not(@direction)">
            <xsl:call-template name="print-warning">
                <xsl:with-param name="msg">Attribute 'direction' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    ...
    <xsl:template match="fo:character">
        ...
        <xsl:if test="not(@character)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'character' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
        ...
        <xsl:choose>
            <xsl:when test="parent::fo:block">
                <xsl:if test="preceding-sibling::*">
                    <xsl:call-template name="print-error">
                        <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' should
                            be the first child of '<xsl:value-of select="name(parent::fo:block)"/>'.
                        </xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
                <xsl:if test="preceding-sibling::text()[normalize-space() != '']">
                    <xsl:call-template name="print-error">
                        <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' cannot
                            be preceded by non-space characters in '<xsl:value-of
                                select="name(parent::fo:block)"/>'. </xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
                <xsl:if test="preceding-sibling::fo:initial-property-set">
                    <xsl:call-template name="print-error">
                        <xsl:with-param name="msg">Only one '<xsl:value-of select="name()"/>'
                            element is permitted in 'xsl:value-of
                            select="name(parent::fo:block)"/&gt;'. </xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
            </xsl:when>
            <xsl:otherwise>
                <xsl:call-template name="print-error">
                    <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' can only
                        occur inside 'block'. </xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
        ...
        <xsl:choose>
            <xsl:when test="parent::fo:block-container">
                <xsl:if test="preceding-sibling::*">
                    <xsl:call-template name="print-error">
                        <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' should
                            be the first child of '<xsl:value-of
                                select="name(parent::fo:block-container)"/>'. </xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
                <xsl:if test="preceding-sibling::text()[normalize-space() != '']">
                    <xsl:call-template name="print-error">
            ...
            <xsl:otherwise>
                <xsl:call-template name="print-error">
                    <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' can only
                        occur inside 'block-container'. </xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="fo:page-number-citation">
        ...
        <xsl:if test="not(@ref-id)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'ref-id' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="rx:page-index">
        ...
        <xsl:apply-templates select="*[not(self::rx:index-item)]" mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'rx:index-item' elements are permitted in this
                context</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(@ref-key) and not(rx:index-item)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' must have
                    either 'ref-key' attribute, or 'rx:index-item' descendant
                    elements.</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="@ref-key and rx:index-item">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' can have either
                    'ref-key' attribute or '<xsl:value-of select="name(rx:index-item[1])"/>'
                    descendant elements, but not both.</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="rx:index-item">
        ...
        <xsl:if test="not(@ref-key)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'ref-key' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="rx:begin-index-range">
        ...
        <xsl:if test="not(@rx:key)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'rx:key' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:choose>
            <xsl:when test="not(@id)">
                <xsl:call-template name="print-error">
                    <xsl:with-param name="msg">Attribute 'id' is required for '<xsl:value-of
                            select="name()"/>'. </xsl:with-param>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                ...
                <xsl:if test="count(preceding::rx:begin-index-range[@id = $id]) = 1">
                    <xsl:call-template name="print-error">
                        <xsl:with-param name="msg">Two or more '<xsl:value-of select="name()"/>'
                            elements with id='<xsl:value-of select="@id"/>'; 'id' should be unique
                            within the document.</xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:call-template name="print-error">
                            <xsl:with-param name="msg">Unmatched '<xsl:value-of select="name()"/>'
                                element with id='<xsl:value-of select="@id"/>': no correspondent
                                'rx:end-index-range' found. </xsl:with-param>
                        </xsl:call-template>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:if test="count(preceding::rx:end-index-range[@ref-id = $refid]) = 1">
                    <xsl:call-template name="print-error">
                        <xsl:with-param name="msg">Two or more '<xsl:value-of select="name()"/>'
                            elements with ref-id='<xsl:value-of select="@ref-id"
                            />'.</xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
                <xsl:choose>
                    <xsl:when test="following::rx:begin-index-range[@id = $refid]">
                        <xsl:call-template name="print-warning">
                            <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>'
                                with ref-id='<xsl:value-of select="@ref-id"/>' precedes its matching
                                    '<xsl:value-of
                                    select="name(following::rx:begin-index-range[@id = $refid][1])"
                                />'. </xsl:with-param>
                        </xsl:call-template>
                    </xsl:when>
                    ...
                    <xsl:otherwise>
                        <xsl:call-template name="print-error">
                            <xsl:with-param name="msg">Unmatched '<xsl:value-of select="name()"/>'
                                element with ref-id='<xsl:value-of select="@ref-id"/>': no
                                correspondent 'rx:begin-index-range' found. </xsl:with-param>
                        </xsl:call-template>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
    ...
    <xsl:template match="fo:table-and-caption">
        ...
        <xsl:call-template name="no-text"/>
        <xsl:apply-templates
            select="*[not(self::fo:table-caption or self::fo:table or self::fo:marker)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'table-caption' and 'table' elements are permitted in
                this context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(fo:table)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element 'table' is required inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:table) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:table[1])"/>' element inside '<xsl:value-of select="name()"
                    />'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:table-caption) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:table-caption[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:table-caption[preceding-sibling::fo:table]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name(fo:table-caption[1])"
                    />' must precede '<xsl:value-of select="name(fo:table[1])"/>' inside
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    ...
    <xsl:template match="fo:table">
        ...
        <xsl:call-template name="no-text"/>
        <xsl:apply-templates
            select="*[not(self::fo:table-column or self::fo:table-header or self::fo:table-footer or self::fo:table-body or self::fo:marker)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'table-column' elements, row group elements, and
                markers are permitted in this context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(fo:table-body)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">At least one 'table-body' is required inside
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:table-header) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:table-header[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:table-footer) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:table-footer[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if
            test="fo:table-column[preceding-sibling::*[not(self::fo:table-column or self::fo:marker)]]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">'<xsl:value-of select="name(fo:table-column[1])"/>'
                    elements should be located before any other element in '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:table-header[preceding-sibling::fo:table-footer]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name(fo:table-header[1])"
                    />' should be located before '<xsl:value-of select="name(fo:table-footer[1])"/>'
                    in '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:table-header[preceding-sibling::fo:table-body]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name(fo:table-header[1])"
                    />' should be located before '<xsl:value-of select="name(fo:table-body[1])"/>'
                    in '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:table-body[following-sibling::*[not(self::fo:table-body)]]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">'<xsl:value-of select="name(fo:table-body[1])"/>'
                    elements should be located last in '<xsl:value-of select="name()"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    ...
    <xsl:template match="fo:table-body | fo:table-header | fo:table-footer">
        ...
        <xsl:call-template name="no-text"/>
        <xsl:apply-templates
            select="*[not(self::fo:table-row or self::fo:table-cell or self::fo:marker)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'table-row', 'table-cell', or 'marker' elements are
                permitted in this context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="$strictness &gt; 0">
            <xsl:if test="not(*)">
                <xsl:call-template name="print-error">
                    <xsl:with-param name="msg">Empty '<xsl:value-of select="name()"/>'.
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:if>
        </xsl:if>
        <xsl:if test="fo:table-row and fo:table-cell">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">'<xsl:value-of select="name(fo:table-row[1])"/>' and
                        '<xsl:value-of select="name(fo:table-cell[1])"/>' elements should not be
                    mixed inside '<xsl:value-of select="name()"/>' element. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="fo:table-row">
        ...
        <xsl:apply-templates select="*[not(self::fo:table-cell)]" mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'table-cell' elements are permitted in this
                context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if
            test="not(parent::fo:table-body or parent::fo:table-header or parent::fo:table-footer)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' should be
                    wrapped in 'table-body', 'table-header', or 'table-footer'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="fo:list-block">
        ...
        <xsl:apply-templates select="*[not(self::fo:list-item or self::fo:marker)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'list-item' or 'marker' elements are permitted in
                this context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="fo:list-item">
        ...
        <xsl:call-template name="no-text"/>
        <xsl:apply-templates
            select="*[not(self::fo:list-item-label or self::fo:list-item-body or self::fo:marker)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Element '<xsl:value-of select="name()"/>' shall consist of
                one 'list-item-label' and one 'list-item-body'.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(parent::fo:list-block)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' should be a
                    child of 'list-block'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="not(fo:list-item-label)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element 'list-item-label' is required inside
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="not(fo:list-item-body)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element 'list-item-body' is required inside
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:list-item-label) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:list-item-label[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:list-item-body) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:list-item-body[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:list-item-body[following-sibling::fo:list-item-label]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of
                        select="name(fo:list-item-label[1])"/>' must precede '<xsl:value-of
                        select="name(fo:list-item-body[1])"/>' inside '<xsl:value-of select="name()"
                    />'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if
            test="fo:list-item-label and not(fo:list-item-label/@start-indent) and not(fo:list-item-label/@margin) and not(fo:list-item-label/@margin-left) and not(fo:list-item-label/@margin-right) and not(fo:list-item-label/@margin-top) and not(fo:list-item-label/@margin-bottom) and fo:list-item-body and not(fo:list-item-body/@start-indent) and not(fo:list-item-body/@margin) and not(fo:list-item-body/@margin-left) and not(fo:list-item-body/@margin-right) and not(fo:list-item-body/@margin-top) and not(fo:list-item-body/@margin-bottom)">
            <xsl:call-template name="print-warning">
                <xsl:with-param name="msg">Start-indent is not specified neither on '<xsl:value-of
                        select="name(fo:list-item-label[1])"/>' nor on '<xsl:value-of
                        select="name(fo:list-item-body[1])"/>': list subcomponents will
                    overlap</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if
            test="fo:list-item-label and not(fo:list-item-label/@end-indent) and not(fo:list-item-label/@margin) and not(fo:list-item-label/@margin-left) and not(fo:list-item-label/@margin-right) and not(fo:list-item-label/@margin-top) and not(fo:list-item-label/@margin-bottom) and fo:list-item-body and not(fo:list-item-body/@end-indent) and not(fo:list-item-body/@margin) and not(fo:list-item-body/@margin-left) and not(fo:list-item-body/@margin-right) and not(fo:list-item-body/@margin-top) and not(fo:list-item-body/@margin-bottom)">
            <xsl:call-template name="print-warning">
                <xsl:with-param name="msg">End-indent is not specified neither on '<xsl:value-of
                        select="name(fo:list-item-label[1])"/>' nor on '<xsl:value-of
                        select="name(fo:list-item-body[1])"/>': list subcomponents will
                    overlap</xsl:with-param>
            </xsl:call-template>
    ...
    <xsl:template match="fo:footnote">
        ...
        <xsl:apply-templates select="*[not(self::fo:inline or self::fo:footnote-body)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Element '<xsl:value-of select="name()"/>' shall consist of
                one 'inline' and one 'footnote-body'.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(fo:inline)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element 'inline' is required inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="not(fo:footnote-body)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element 'footnote-body' is required inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:inline) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:inline[1])"/>' element inside '<xsl:value-of select="name()"
                    />'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:footnote-body) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:footnote-body[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:footnote-body[following-sibling::fo:inline]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name(fo:inline[1])"/>'
                    must precede '<xsl:value-of select="name(fo:footnote-body[1])"/>' inside
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template match="fo:marker">
        ...
        <xsl:choose>
            <xsl:when test="not(@marker-class-name)">
                <xsl:call-template name="print-error">
                    <xsl:with-param name="msg">Attribute 'marker-class-name' is required for
                            '<xsl:value-of select="name()"/>'. </xsl:with-param>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                ...
                <xsl:if test="preceding-sibling::fo:marker[@marker-class-name = $marker-class-name]">
                    <xsl:call-template name="print-error">
                        <xsl:with-param name="msg">Sibling <xsl:value-of select="name()"/> elements
                            have the same value of 'marker-class-name' attribute ("<xsl:value-of
                                select="@marker-class-name"/>"). </xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
            </xsl:otherwise>
        </xsl:choose>
        <xsl:if test="preceding-sibling::*[not(self::fo:marker)]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">'<xsl:value-of select="name()"/>' elements should be
                    initial children of their parents. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="preceding-sibling::text()[normalize-space() != '']">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' cannot be
                    preceded by non-space text nodes. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="not(ancestor::fo:flow)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' should be a
                    descendant of 'flow'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="ancestor::fo:multi-case">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' cannot be used
                    inside '<xsl:value-of select="name(ancestor::fo:multi-case[1])"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="ancestor::fo:float">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' cannot be used
                    inside '<xsl:value-of select="name(ancestor::fo:float[1])"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="ancestor::fo:footnote">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' cannot be used
                    inside '<xsl:value-of select="name(ancestor::fo:footnote[1])"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    ...
    <xsl:template match="fo:retrieve-marker">
        ...
        <xsl:if test="not(@retrieve-class-name)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'retrieve-class-name' is required for
                        '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="not(ancestor::fo:static-content)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' should be a
                    descendant of 'static-content'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="fo:multi-switch">
        ...
        <xsl:apply-templates select="*[not(self::fo:multi-case)]" mode="report-intrusive-elements">
            <xsl:with-param name="reason">Only 'multi-case' elements are permitted in this
                context.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="fo:multi-case">
        ...
        <xsl:if test="not(parent::fo:multi-switch)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' should be a
                    child of 'multi-switch'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="fo:multi-toggle">
        ...
        <xsl:if test="not(ancestor::fo:multi-case)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' can be used
                    only inside 'multi-case'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="fo:multi-properties">
        ...
        <xsl:apply-templates select="*[not(self::fo:multi-property-set or self::fo:wrapper)]"
            mode="report-intrusive-elements">
            <xsl:with-param name="reason">Element '<xsl:value-of select="name()"/>' shall consist of
                one or more 'multi-property-set' elements, followed by a 'wrapper'.</xsl:with-param>
        </xsl:apply-templates>
        <xsl:if test="not(fo:multi-property-set)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">At least one 'multi-property-set' element should be
                    specified in '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="not(fo:wrapper)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element 'wrapper' is required inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="count(fo:wrapper) &gt; 1">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">There can be only one '<xsl:value-of
                        select="name(fo:wrapper[1])"/>' element inside '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="fo:wrapper[following-sibling::fo:multi-property-set]">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name(fo:wrapper[1])"/>'
                    must be located after all '<xsl:value-of select="name(fo:multi-property-set[1])"
                    />' elements inside '<xsl:value-of select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates select="*"/>
    </xsl:template>
    ...
    <xsl:template match="fo:multi-property-set">
        ...
        <xsl:if test="not(@active-state)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'active-state' is required for '<xsl:value-of
                        select="name()"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="not(parent::fo:multi-properties)">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' should be a
                    child of 'multi-properties'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="block-level">
        <xsl:call-template name="no-text">
            <xsl:with-param name="reason">Only block-level elements are permitted in this
                context.</xsl:with-param>
        </xsl:call-template>
        <xsl:apply-templates
            select="*[not(self::fo:block or self::fo:block-container or self::fo:list-block or self::fo:table or self::fo:table-and-caption or self::fo:float or self::fo:wrapper or self::fo:marker or self::fo:retrieve-marker or self::fo:multi-switch or self::fo:multi-properties or self::rx:ruler or self::rx:begin-index-range or self::rx:end-index-range or self::rx:flow-section)]"
    ...
    <xsl:template name="out-of-line">
        ...
        <xsl:if test="ancestor::fo:float">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' cannot be a
                    descendant of '<xsl:value-of select="name(ancestor::fo:float[1])"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="ancestor::fo:footnote-body">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' cannot be a
                    descendant of '<xsl:value-of select="name(ancestor::fo:footnote-body[1])"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if test="ancestor::fo:marker">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' cannot be a
                    descendant of '<xsl:value-of select="name(ancestor::fo:marker[1])"/>'.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:if
            test="ancestor::*[@position = 'absolute' or @absolute-position = 'absolute' or @position = 'fixed' or @absolute-position = 'fixed']">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' cannot be a
                    descendant of an absolutely positioned '<xsl:value-of
                        select="name(ancestor::*[@position = 'absolute' or @absolute-position = 'absolute' or @position = 'fixed' or @absolute-position = 'fixed'][1])"
                    />'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="no-text">
        ...
        <xsl:if test="text()[normalize-space(.) != '']">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg"> Element '<xsl:value-of select="name()"/>' cannot have
                    non-empty text descendants here. <xsl:value-of
    ...
    <xsl:template name="empty-element">
        <xsl:if test="* | text()">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Element '<xsl:value-of select="name()"/>' must be empty.
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="*" mode="report-intrusive-elements" priority="-1">
        ...
        <xsl:choose>
            <xsl:when
                test="$uri = 'http://www.w3.org/1999/XSL/Format' or $uri = 'http://www.renderx.com/XSL/Extensions'">
                <xsl:call-template name="print-error">
                    <xsl:with-param name="msg"> Element '<xsl:value-of select="name()"/>' cannot be
                        a child of '<xsl:value-of select="name(..)"/>'. <xsl:if test="$reason != ''">
                            <xsl:value-of select="$reason"/>
                        </xsl:if>
                    </xsl:with-param>
                </xsl:call-template>
            ...
            <xsl:when test="$uri = ''">
                <xsl:call-template name="print-error">
                    <xsl:with-param name="msg"> Element '<xsl:value-of select="name()"/>' belonging
                        to the anonymous namespace cannot occur in an XSL-FO document outside
                        'instream-foreign-object' elements. </xsl:with-param>
                </xsl:call-template>
            </xsl:when>
            <xsl:when test="$uri = 'http://www.w3.org/2000/svg'">
                <xsl:if test="$strictness &gt; 0">
                    <xsl:call-template name="print-warning">
                        <xsl:with-param name="msg"> SVG element '<xsl:value-of select="name()"/>' is
                            located outside 'instream-foreign-object' element. </xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
            </xsl:when>
            <xsl:otherwise>
                <xsl:if test="$strictness &gt; 1">
                    <xsl:call-template name="print-warning">
                        <xsl:with-param name="msg"> Element '<xsl:value-of select="name()"/>'
                            belongs to an unknown namespace <xsl:value-of select="$uri"/>
                        </xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
            </xsl:otherwise>
    ...
    <xsl:template match="fo:wrapper/* | fo:multi-properties/* | fo:multi-case/*"
        ...
        <xsl:call-template name="print-error">
            <xsl:with-param name="msg"> Element '<xsl:value-of select="name()"/>' cannot be a
                descendant of '<xsl:value-of
                    select="name(ancestor::*[not(self::fo:wrapper or self::fo:multi-switch or self::fo:multi-case or self::fo:multi-properties)][1])"
                />' through wrapper elements. <xsl:if test="$reason != ''">
                    <xsl:value-of select="$reason"/>
                </xsl:if>
            </xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template
        match="fo:wrapper/* | fo:inline/* | fo:basic-link/* | fo:bidi-override/* | fo:multi-toggle/* | fo:multi-properties/* | fo:multi-case/*"
        mode="report-intrusive-elements-inline">
        <xsl:param name="reason" select="''"/>
        <xsl:call-template name="print-error">
            <xsl:with-param name="msg"> Element '<xsl:value-of select="name()"/>' cannot be a
                descendant of '<xsl:value-of
                    select="name(ancestor::*[not(self::fo:wrapper or self::fo:multi-switch or self::fo:multi-case or self::fo:multi-toggle or self::fo:multi-properties or self::fo:inline)][1])"
                />' through wrapper or inline elements. <xsl:if test="$reason != ''">
                    <xsl:value-of select="$reason"/>
                </xsl:if>
            </xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template
        match="@*[not(parent::fo:root) and not(ancestor::fo:page-sequence) and not(parent::*/parent::fo:simple-page-master)]"
        priority="4">
        <xsl:call-template name="complain-attribute"/>
    </xsl:template>
    ...
    <xsl:template name="complain-attribute">
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="$uri = '' or $uri = 'http://www.w3.org/1999/XSL/Format' or $uri = 'http://www.renderx.com/XSL/Extensions'">
                <xsl:call-template name="print-error">
                    <xsl:with-param name="msg">Attribute '<xsl:value-of select="name()"/>' cannot
                        occur at element '<xsl:value-of select="name(..)"/>'. </xsl:with-param>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                <xsl:if test="$strictness &gt; 1">
                    <xsl:call-template name="print-warning">
                        <xsl:with-param name="msg">Attribute '<xsl:value-of select="name()"/>' at
                            element '<xsl:value-of select="name(..)"/>' belongs to an unknown
    ...
    <xsl:template name="disable-on-atomic-inlines">
        <xsl:if
            test="parent::fo:character or parent::fo:page-number or parent::fo:page-number-citation or parent::rx:page-number-citation-last or parent::fo:initial-property-set">
            <xsl:call-template name="complain-attribute"/>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="enumerated-values">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name="print-error">
                    <xsl:with-param name="msg">Attribute '<xsl:value-of select="name()"/>' cannot
                        have a value of "<xsl:value-of select="$value"/>". </xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="check-url">
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="starts-with($val, 'url(') and substring($val, string-length($val)) = ')'"/>
            <xsl:otherwise>
                <xsl:call-template name="print-warning">
                    <xsl:with-param name="msg">Attribute '<xsl:value-of select="name()"/>' cannot
                        have a value of "<xsl:value-of select="."/>": should be either 'inherit' or
                        a URI: <xsl:value-of select="name()"/>="url(...)".</xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="@source-document" priority="5">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name="print-error">
                    <xsl:with-param name="msg">Attribute '<xsl:value-of select="name()"/>' cannot
                        have a value of "<xsl:value-of select="."/>": should be 'inherit', 'none',
                        or one or more URIs: source="url(...) [url(...) ...]".</xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="@elevation">
        <xsl:call-template name="quantitative-values">
            <xsl:with-param name="valuelist"> below level above higher lower inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="@speech-rate">
        <xsl:call-template name="quantitative-values">
            <xsl:with-param name="valuelist"> x-slow slow medium fast x-fast faster slower inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="@volume">
        <xsl:call-template name="quantitative-values">
            <xsl:with-param name="valuelist"> silent x-soft soft medium loud x-loud inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template priority="3" match="@background-repeat">
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> repeat repeat-x repeat-y no-repeat inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template priority="3"
        ...
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> none hidden dotted dashed solid double groove ridge
                inset outset inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template priority="3"
    ...
    <xsl:template
        match="@border-before-width.conditionality | @border-after-width.conditionality | @border-start-width.conditionality | @border-end-width.conditionality">
        <xsl:call-template name="enumerated-values">
    ...
    <xsl:template
        match="@padding-before.conditionality | @padding-after.conditionality | @padding-start.conditionality | @padding-end.conditionality">
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> discard retain </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="@font-size">
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="quantitative-values">
            <xsl:with-param name="valuelist"> xx-small x-small small medium large x-large xx-large
                larger smaller inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="@font-stretch">
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="quantitative-values">
            <!-- this is kinda sorta extension -->
            <xsl:with-param name="valuelist"> normal wider narrower ultra-condensed extra-condensed
                condensed semi-condensed semi-expanded expanded extra-expanded ultra-expanded
                inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="@font-style">
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> normal italic oblique backslant inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template
        match="@space-before | @space-before.minimum | @space-before.optimum | @space-before.maximum | @space-after | @space-after.minimum | @space-after.optimum | @space-after.maximum | @space-start | @space-start.minimum | @space-start.optimum | @space-start.maximum | @space-end | @space-end.minimum | @space-end.optimum | @space-end.maximum">
        <xsl:call-template name="quantitative-values">
            <xsl:with-param name="valuelist"> inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template
        match="@space-before.conditionality | @space-after.conditionality | @space-start.conditionality | @space-end.conditionality">
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> discard retain </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template
        match="@space-before.precedence | @space-after.precedence | @space-start.precedence | @space-end.precedence">
        <xsl:call-template name="quantitative-values">
    ...
    <xsl:template match="@alignment-adjust">
        <xsl:call-template name="quantitative-values">
            <xsl:with-param name="valuelist"> auto baseline before-edge text-before-edge middle
                central after-edge text-after-edge ideographic alphabetic hanging mathematical top
                bottom text-top text-bottom inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="@dominant-baseline">
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> auto use-script no-change reset-size ideographic
                alphabetic hanging mathematical central middle text-after-edge text-before-edge
                inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template
        match="@block-progression-dimension | @block-progression-dimension.minimum | @block-progression-dimension.optimum | @block-progression-dimension.maximum | @inline-progression-dimension | @inline-progression-dimension.minimum | @inline-progression-dimension.optimum | @inline-progression-dimension.maximum | @height | @width">
        <xsl:call-template name="disable-on-atomic-inlines"/>
        <xsl:call-template name="quantitative-values">
            <xsl:with-param name="valuelist"> auto inherit </xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template match="@scaling-method">
        <xsl:call-template name="disable-on-atomic-inlines"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> auto integer-pixels resample-any-method inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="@line-height-shift-adjustment">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> consider-shifts disregard-shifts inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="@line-stacking-strategy">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> line-height font-height max-height inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="@linefeed-treatment">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> ignore preserve treat-as-space
                treat-as-zero-width-space inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="@white-space-treatment">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> ignore preserve ignore-if-before-linefeed
                ignore-if-after-linefeed ignore-if-surrounding-linefeed inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="@text-align">
        ...
        <xsl:if
            test="string-length($token) != 0 and string-length(translate($token, $keyword-charset, '')) = 0">
            <xsl:call-template name="enumerated-values">
                <xsl:with-param name="valuelist"> start center end justify inside outside left right
                    inherit </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    <xsl:template match="@text-align-last">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> relative start center end justify inside outside left
                right inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template
        match="@letter-spacing.minimum | @letter-spacing.optimum | @letter-spacing.maximum | @letter-spacing | @word-spacing.minimum | @word-spacing.optimum | @word-spacing.maximum | @word-spacing">
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="quantitative-values">
            <xsl:with-param name="valuelist"> normal inherit </xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template match="@text-transform">
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> capitalize uppercase lowercase none inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template
        match="fo:color-profile/@rendering-intent | fo:declarations/@rendering-intent | fo:root/@rendering-intent"
        priority="5">
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> auto perceptual relative-colorimetric saturation
                absolute-colorimetric inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="@clear">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> start end left right both none inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="@float">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> before start end left right inside outside none
                inherit </xsl:with-param>
        </xsl:call-template>
        <xsl:if test="$strictness &gt; 1">
            <xsl:if test=". = 'inside' or . = 'outside'">
                <xsl:call-template name="print-warning">
                    <xsl:with-param name="msg">Value "<xsl:value-of select="."/>" for 'float'
                        attribute is a RenderX extension.</xsl:with-param>
                </xsl:call-template>
            </xsl:if>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="@break-after | @break-before">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> auto column page even-page odd-page inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template
    ...
    <xsl:template priority="3" match="@overflow">
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> visible hidden scroll error-if-overflow auto inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="@span">
        <xsl:if test="ancestor::fo:static-content">
            <xsl:call-template name="print-error">
                <xsl:with-param name="msg">Attribute 'span' cannot be used inside '<xsl:value-of
                        select="name(ancestor::fo:static-content[1])"/>'. </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:call-template name="disable-on-atomic-inlines"/>
        <xsl:call-template name="disable-on-graphics"/>
    ...
    <xsl:template
        match="@leader-length | @leader-length.minimum | @leader-length.optimum | @leader-length.maximum">
        <xsl:call-template name="disable-on-atomic-inlines"/>
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="quantitative-values">
            <xsl:with-param name="valuelist"> inherit </xsl:with-param>
    ...
    <xsl:template match="@rule-style">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> none dotted dashed solid double groove ridge inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="fo:retrieve-marker/@retrieve-position">
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> first-starting-within-page first-including-carryover
                last-starting-within-page last-ending-within-page </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="fo:page-sequence/@force-page-count">
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> auto even odd end-on-even end-on-odd no-force
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="fo:root/@media-usage">
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> auto paginate bounded-in-one-dimension unbounded
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template priority="5"
    ...
    <xsl:template
        match="@border-after-precedence | @border-before-precedence | @border-end-precedence | @border-start-precedence">
        <xsl:call-template name="disable-on-atomic-inlines"/>
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="quantitative-values">
    ...
    <xsl:template match="@border-collapse">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> collapse collapse-with-precedence separate inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template
        match="@border-separation | @border-separation.block-progression-direction | @border-separation.inline-progression-direction">
        <xsl:call-template name="disable-on-atomic-inlines"/>
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="quantitative-values">
            <xsl:with-param name="valuelist"> inherit </xsl:with-param>
    ...
    <xsl:template match="@caption-side">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> before after start end top bottom left right inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template
        match="fo:table-column/@number-columns-spanned | fo:table-cell/@number-columns-spanned">
        <xsl:call-template name="quantitative-values"/>
    </xsl:template>
    ...
    <xsl:template
        match="fo:table/@table-omit-footer-at-break | fo:table/@table-omit-header-at-break | fo:table/@rx:table-omit-initial-header | fo:table/@rx:table-omit-final-footer">
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> true false </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template priority="3" match="@writing-mode">
        ...
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> lr-tb rl-tb tb-rl tb-lr lr-alternating-rl-tb
                lr-inverting-rl-tb lr-bt rl-bt bt-rl bt-lr lr-alternating-rl-bt lr-inverting-rl-bt
                tb-lr-in-lr-pairs lr rl tb inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template
        match="fo:external-graphic/@content-type | fo:instream-foreign-object/@content-type | rx:background-content-type"
        priority="3">
        <xsl:variable name="val" select="normalize-space()"/>
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name="print-warning">
                    <xsl:with-param name="msg">Attribute '<xsl:value-of select="name()"/>' cannot
                        have a value of "<xsl:value-of select="."/>": if not 'auto', it should be
                        prefixed by either 'namespace:' or 'content-type:'.</xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="@position">
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> static relative absolute fixed inherit
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="@vertical-align">
        <xsl:call-template name="quantitative-values">
            <xsl:with-param name="valuelist"> baseline middle sub super text-top text-bottom top
                bottom inherit </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template
        match="@rx:range-separator | rx:page-index/@range-separator | rx:index-item/@range-separator">
        <xsl:call-template name="disable-on-atomic-inlines"/>
        <xsl:call-template name="disable-on-graphics"/>
    </xsl:template>
    <xsl:template
        match="@rx:merge-subsequent-page-numbers | rx:page-index/@merge-subsequent-page-numbers | rx:index-item/@merge-subsequent-page-numbers">
        <xsl:call-template name="disable-on-atomic-inlines"/>
        <xsl:call-template name="disable-on-graphics"/>
        <xsl:call-template name="enumerated-values">
            <xsl:with-param name="valuelist"> false true </xsl:with-param>
