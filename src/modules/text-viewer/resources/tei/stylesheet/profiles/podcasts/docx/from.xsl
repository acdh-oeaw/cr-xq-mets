...
<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0"
    ...
    exclude-result-prefixes="cp ve o r m v wp w10 w wne mml media       tbx pic rel a creativeCommons       itunes tei teidocx xs iso">
    ...
    <xsl:template name="create-tei-header">
        ...
        <xsl:analyze-string select="$filename" regex="([^/]+)/([^/]+)/([^\.]+)\.?.*">
            <xsl:matching-substring>
                ...
                <xsl:for-each select="document($lookup)">
                    <xsl:choose>
                            ...
                            <xsl:for-each select="//item[contains(enclosure/@url, $file)]">
                                <teiHeader>
                                    <fileDesc>
                                        ...
                                        <publicationStmt>
                                            <idno type="feed">http://rss.oucs.ox.ac.uk<xsl:value-of
                                                  select="$feed"/>/rss20.xml</idno>
                                            <idno type="guid">
                                            ...
                                            <publisher>
                                                <graphic rend="noindent"
                                                  url="file:///usr/share/xml/tei/stylesheet/profiles/podcasts/docx/TranscriptHeader.jpg"
                                                  width="100%"/>
                                            </publisher>
                                        </publicationStmt>
                                        <notesStmt>
                        ...
                        <xsl:otherwise>
                            <xsl:message terminate="yes">Error finding <xsl:value-of select="$file"
                                /> in <xsl:value-of select="$lookup"/> (from <xsl:value-of
            ...
            <xsl:non-matching-substring>
                <xsl:message terminate="yes">Error finding a valid full name in <xsl:value-of
                ...
                <xsl:copy>
                    <xsl:variable name="body">
                        <xsl:for-each-group select="*"
                            group-starting-with="tei:p[contains(., 'START AUDIO') or contains(., 'END AUDIO')]">
                            <xsl:choose>
