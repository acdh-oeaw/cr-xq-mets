...
<xsl:stylesheet xmlns:tei="http://www.tei-c.org/ns/1.0"
    ...
    exclude-result-prefixes="tei m" version="2.0">
    ...
    <xsl:template match="tei:hi[@rend = 'bo']"> \textbf{<xsl:apply-templates/>} </xsl:template>
    <xsl:template match="tei:hi[@rend = 'smcap']"> \textsc{<xsl:apply-templates/>} </xsl:template>
    <xsl:template match="tei:hi[@rend = 'del']"> \sout{<xsl:apply-templates/>} </xsl:template>
    <xsl:template match="tei:hi[@rend = 'ul2']"> \uuline{<xsl:apply-templates/>} </xsl:template>
    <xsl:template match="tei:hi[@rend = 'ulw']"> \uwave{<xsl:apply-templates/>} </xsl:template>

    <!-- do weird list types -->
    <xsl:template match="tei:list[@type = 'number']"> \begin{enumerate} <xsl:apply-templates
            select="tei:item"/> \end{enumerate} </xsl:template>
    <xsl:template match="tei:list[@type = 'simple']"> \begin{list}{\quad}{} <xsl:apply-templates
            select="tei:item"/> \end{list} </xsl:template>
