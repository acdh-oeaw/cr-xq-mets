...
<xsl:stylesheet xmlns:con="http://schemas.openxmlformats.org/package/2006/content-types"
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="relations">
            <xsl:apply-templates
                select="doc(concat($wordDirectory, '/word/_rels/document.xml.rels'))"
                mode="relations"/>
        </xsl:variable>
        <xsl:variable name="content-types">
            <xsl:apply-templates mode="content-types"
                select="doc(concat($wordDirectory, '/Content_Types.xml'))"/>
        </xsl:variable>
    ...
    <xsl:template name="write-out-content-types">
        ...
        <xsl:if test="$debug = 'true'">
            <xsl:message>Writing out <xsl:value-of
                    select="concat($wordDirectory, '/Content_Types_new.xml')"/>
            </xsl:message>
        </xsl:if>
        <xsl:result-document href="{concat($wordDirectory,'/Content_Types_new.xml')}"
            standalone="yes">
    ...
    <xsl:template name="write-out-relations">
        ...
        <xsl:if test="$debug = 'true'">
            <xsl:message>Writing out <xsl:value-of
                    select="concat($wordDirectory, 'word/_rels/document_new.xml.rels')"/>
            </xsl:message>
        </xsl:if>
    ...
    <xsl:template name="write-out-comments">
        ...
        <xsl:if test="$debug = 'true'">
            <xsl:message>Writing out <xsl:value-of
                    select="concat($wordDirectory, 'word/_rels/comments.xml')"/>
            </xsl:message>
        </xsl:if>
        <xsl:result-document href="{concat($wordDirectory,'/word/comments.xml')}" standalone="yes">
            <w:comments xmlns:mo="http://schemas.microsoft.com/office/mac/office/2008/main"
