...
<xsl:stylesheet xmlns:exist="http://exist.sourceforge.net/NS/exist"
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mml="http://www.w3.org/1998/Math/MathML"
    ...
    -->
    <xsl:output method="xml" doctype-public="-//NLM//DTD Journal Publishing DTD v3.0 20080202//EN"
        doctype-system="http://dtd.nlm.nih.gov/publishing/3.0/journalpublishing3.dtd"
        xpath-default-namespace="" indent="yes"/>
    <xsl:template match="/">
     ...
     and article metadata. First, journal metadata: -->
                <xsl:element name="journal-meta">
                    <journal-id journal-id-type="publisher">
                        <xsl:choose>
                            <xsl:when
                                test="TEI/teiHeader[1]/fileDesc[1]/publicationStmt[1]/publisher[1]/choice[1]/abbr[1]">
                                <xsl:value-of
                                    select="TEI/teiHeader[1]/fileDesc[1]/publicationStmt[1]/publisher[1]/choice[1]/abbr[1]"
                                />
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of
                                    select="TEI/teiHeader[1]/fileDesc[1]/publicationStmt[1]/publisher[1]"
                                />
                            </xsl:otherwise>
                        </xsl:choose>
                    </journal-id>
                    <issn>
                        <xsl:value-of
                            select="TEI/teiHeader[1]/fileDesc[1]/seriesStmt[1]/idno[@type = 'issn']"
                        />
                    </issn>
                    <publisher>
                        <publisher-name>
                            <xsl:choose>
                                <xsl:when
                                    test="TEI/teiHeader[1]/fileDesc[1]/publicationStmt[1]/publisher[1]/choice/abbr">
                                    <xsl:value-of
                                        select="TEI/teiHeader[1]/fileDesc[1]/publicationStmt[1]/publisher[1]/choice/abbr"/>
                                    <xsl:text> (</xsl:text>
                                    <xsl:apply-templates
                                        select="TEI/teiHeader[1]/fileDesc[1]/publicationStmt[1]/publisher[1]/choice/expan"/>
                                    <xsl:text>)</xsl:text>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of
                ...
                <xsl:element name="article-meta">
                    ...
                    <title-group>
                        ...
                        <subtitle>
                            <xsl:apply-templates
                                select="TEI/teiHeader[1]/fileDesc[1]/titleStmt[1]/title[@type = 'sub']"
                            />
                        </subtitle>
                        <alt-title alt-title-type="runningRecto">
                            <xsl:apply-templates
                                select="TEI/teiHeader[1]/fileDesc[1]/titleStmt[1]/title[@type = 'runningRecto']"
                            />
                        </alt-title>
                        <alt-title alt-title-type="runningVerso">
                            <xsl:apply-templates
                                select="TEI/teiHeader[1]/fileDesc[1]/titleStmt[1]/title[@type = 'runningVerso']"
                            />
                        </alt-title>
                    </title-group>
                    <contrib-group>
                    ...
                    <xsl:call-template name="dateToDMYTags">
                        <xsl:with-param name="inDate"
                            select="TEI/teiHeader[1]/fileDesc[1]/publicationStmt[1]/date[1]/@when"/>
                        <xsl:with-param name="outTagName">pub-date</xsl:with-param>
                    </xsl:call-template>
                    <volume>
                        <xsl:apply-templates
                            select="TEI/teiHeader[1]/fileDesc[1]/seriesStmt[1]/idno[@type = 'vol']"
                        />
                    </volume>
                    <issue>
                        <xsl:apply-templates
                            select="TEI/teiHeader[1]/fileDesc[1]/seriesStmt[1]/idno[@type = 'issue']"
                        />
                    </issue>
                    <elocation-id>
                        <xsl:value-of
                            select="TEI/teiHeader[1]/fileDesc[1]/publicationStmt[1]/idno[@type = 'itemNo']/@n"
                        />
                    </elocation-id>
...
journals), or it could be s (series title). We need to handle each separately.-->
                    <xsl:for-each
                        select="monogr/title[@level = 'm' or @level = 'u' or @level = 'j']">
                        <source>
                            <xsl:value-of select="."/>
                        </source>
                    </xsl:for-each>
...
at the same time explain them. -->
    <xsl:template match="soCalled">
        <styled-content style-type="scare quotes"
            style="::before: content(open-quote); ::after: content(close-quote);">
            <xsl:apply-templates/>
        </styled-content>
    </xsl:template>
    ...
    <xsl:template match="tag">
        <styled-content style-type="XML tag"
            style="::before: content(&lt;); ::after: content(&gt;); font-family: monospace;">
            <xsl:apply-templates/>
        </styled-content>
    </xsl:template>
