...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    -->
    <xsl:output method="xml" doctype-public="-//NLM//DTD Journal Publishing DTD v2.3 20070202//EN"
...
will need to be handled separately. -->
    <xsl:template match="styled-content[@style-type = 'scare quotes']"> "<xsl:apply-templates/>" </xsl:template>
...
fixed list of items.  Some of our types will not do. -->
    ...
    <xsl:template match="@* | node() | text() | comment() | processing-instruction()" priority="-1">
        <xsl:copy>
            <xsl:apply-templates
                select="@* | node() | text() | comment() | processing-instruction()"/>
        </xsl:copy>
    </xsl:template>
</xsl:stylesheet>
