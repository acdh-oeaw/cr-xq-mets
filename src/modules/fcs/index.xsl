...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="filter-mode" select="
            if (starts-with($filter, '*')) then
                'contains'
            else
                'starts-with'"/>
            ...
            <xsl:choose>
                <xsl:when test="$mode = 'subsequence'">
                    ...
                    <sru:extraResponseData>
                        <xsl:copy-of
                            select="sru:scanResponse/sru:extraResponseData/*[not(local-name() = 'terms')]"
                        />
                    </sru:extraResponseData>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="sru:terms" mode="subsequence">
        ...
        <xsl:variable name="filtered" select="
                ./sru:term[if ($filter-x != '' and not(sru:extraTermData/sru:terms)) then
                    if ($filter-mode = 'starts-with') then
                        (starts-with(lower-case(sru:value), $filter-x) or starts-with(lower-case(sru:displayTerm), $filter-x))
                    else
                        (contains(lower-case(sru:value), $filter-x) or contains(lower-case(sru:displayTerm), $filter-x))
                else
                    true()]"/>
        <xsl:variable name="match-position"
            select="count(sru:term[. = $filtered[1]]/preceding-sibling::sru:term)"/>
                    ...
                    <xsl:choose>
                        <xsl:when test="$sort = 'text'">
                            <xsl:for-each select="$filtered">
                                <xsl:sort select="lower-case(sru:displayTerm)" data-type="text"
                                    order="ascending"/>
                                <xsl:copy-of select="."/>
                        ...
                        <xsl:otherwise>
                            <!-- sort=size -->
                            <xsl:for-each select="$filtered">
                                <xsl:sort select="sru:numberOfRecords" data-type="number"
                                    order="descending"/>
                                <xsl:copy-of select="."/>
        ...
        <xsl:copy>
            ...
            <xsl:variable name="start-pos" select="
                    if ($only-filtered) then
                        0
                    else
                        xs:integer($match-position)"/>
            <xsl:variable name="effective-start-item"
                select="xs:integer($start-item) + $start-pos - xs:integer($response-position) + 1"/>
            <xsl:variable name="effective-end-item"
                select="xs:integer($effective-start-item) + xs:integer($max-items)"/>
            <xsl:apply-templates
                select="$ordered/*[xs:integer(position()) &gt;= xs:integer($effective-start-item) and ((xs:integer(position()) &lt; (xs:integer($effective-end-item))) or xs:integer($max-items) = 0)]">
    ...
    <xsl:template match="sru:term">
        ...
        <xsl:if
            test="not(sru:extraTermData/sru:terms/sru:term and empty($subsequence/sru:terms/sru:term))">
            <xsl:copy>
