    ...
    return 
        <sru:scanResponse xmlns:fcs="http://clarin.eu/fcs/1.0">
            ...
            <sru:extraResponseData>
                ...
                <fcs:countTerms level="total">{count($terms//sru:term[not(./sru:extraTermData/sru:terms)])}</fcs:countTerms>
...
(:	let $resource-pid:= util:eval("$record-data-input/ancestor-or-self::*[1]/data(@cr:"||$config:RESOURCE_PID_NAME||")"):)
    ...
    return 
        ...
        let $dumy := util:log-app("DEBUG",$config:app-name,"$match-ids: "||string-join($match-ids,' '))                    
        let $dumy3 := util:log-app("DEBUG",$config:app-name,"$resourcefragment-pid: '"||$resourcefragment-pid||"'")
    (:   21.1.2015 1 Zeile :)
...
(:  the predicate makes the function by SLOWER by factor 10 !!! [rf:lookup-id(.,$resource-pid, $project-id) = $resourcefragment-pid]:)
    ...
    let $dumy4 := util:log-app("DEBUG",$config:app-name,"$match-ids / rf: "||$resourcefragment-pid||":: "||string-join($matches-to-highlight,' '))
