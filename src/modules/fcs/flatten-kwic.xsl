<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exist="http://exist.sourceforge.net/NS/exist" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:cr="http://aac.ac.at/content_repository" version="2.0"><!--    <xsl:strip-space elements="*"/>--><!--    <xsl:preserve-space elements="tei:seg"/>-->
    ...
    <xsl:template match="text()">
        <xsl:value-of select="if (normalize-space(.) ne '') then . else ' '"/>
    </xsl:template>
    ...
    <xsl:template match="l|tei:l|p|tei:p|head|tei:head|titlePage|tei:titlePage|table|tei:table">
        <xsl:apply-templates select="node() | @*"/>
    ...
    <xsl:template match="*[exist:match]" mode="groupMatches" priority="2">
        <xsl:variable name="seq" select="node()[not(normalize-space(.) = '') or . = ' ']" as="item()*"/>
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:for-each-group select="$seq" group-adjacent="name(.)">
                <xsl:choose>
                    <xsl:when test="current-grouping-key()='exist:match'">
                        <exist:match xml:space="preserve"><xsl:apply-templates select="current-group()" mode="inMatchGroup"/></exist:match>
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template match="exist:match" mode="inMatchGroup" priority="2">
        ...
        <xsl:copy-of select="following-sibling::node()[normalize-space(.) = ''][preceding-sibling::exist:match[1] is $this]"/>-->
    </xsl:template>
</xsl:stylesheet>