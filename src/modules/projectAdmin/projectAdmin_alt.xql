...
let $labels := projectAdmin:labels($form,$lang)
let $instancedata := projectAdmin:instancedata($project-pid, $form)
let $help := projectAdmin:help($form,$lang)   
let $bindings := projectAdmin:bind($project-pid,$form,$vars)
    ...
    <link rel="stylesheet" type="text/css" href="../../shared-resources/resources/css/bootstrap.min.css"/>
    	...
    	<xf:model id="model">
    	   <!-- some basic data which resides on the mets:mets element -->
            ...
            {($labels,$instancedata,$help,$bindings)}
